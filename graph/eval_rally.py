...
# Strip "mlp." prefix if present (model keys) to match nn.Sequential keys
for sd in (lsd, rsd):
    for old_k in list(sd.keys()):
        if old_k.startswith("mlp."):
            sd[old_k[4:]] = sd.pop(old_k)
        elif old_k == "distribution.std_param":
            del sd[old_k]  # not used in inference-only Sequential
