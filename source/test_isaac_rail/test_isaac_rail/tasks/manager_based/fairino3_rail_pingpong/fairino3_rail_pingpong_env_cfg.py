...
from test_isaac_rail.assets.fairino3_v6_rail import FAIRINO3_V6_RAIL_PADDLE_CFG
...
RAIL_ROBOT_POS = (-0.97, 0.0, TABLE_TOP_Z)
...
class CenterlineObservationsCfg:
    ...
    class PolicyCfg(ObsGroup):
        ...
        target_pos = ObsTerm(
            ...
            params={"target": (RIGHT_MID_TARGET_X, 0.0, TABLE_TOP_Z), "robot_cfg": SceneEntityCfg("robot")},
        )
...
class EventCfg:
    ...
    reset_ball = EventTerm(
        func=mdp.reset_ball_from_crossing,
        mode="reset",
        params={
            "asset_cfg": SceneEntityCfg("ball"),
            "net_x": NET_X,
            "pos_mean": (0.53, 0.01, 1.00),
            "pos_std": (0.01, 0.17, 0.05),
            "vel_mean": (-2.55, -0.04, -1.12),
            "vel_std": (0.59, 0.32, 0.70),
            "home_side": "left",
        },
    )
...
class RewardsCfg:
    ...
    paddle_to_intercept = RewTerm(
        ...
        params={
            "robot_cfg": SceneEntityCfg("robot", body_names=["paddle_link"]),
            "ball_cfg": SceneEntityCfg("ball"),
            "intercept_x": LEFT_INTERCEPT_X, "std": 0.30,
        },
    )
    ...
    opponent_compatible = RewTerm(
        func=mdp.opponent_compatible_return_v2, weight=20.0,
        params={
            "asset_cfg": SceneEntityCfg("ball"), "net_x": NET_X,
    ...
    opponent_vz_quality = None  # removed: 只加 compat 单变量，vz_quality 暂不加
    high_return_height = None  # removed: competes with compatible
    centerline_x = None
    predicted_landing = RewTerm(
        func=mdp.predicted_right_table_landing, weight=5.0,
        params={
            "asset_cfg": SceneEntityCfg("ball"),
            "target_xy": (RIGHT_MID_TARGET_X, 0.0),
    ...
    near_net_landing_speed = RewTerm(
        ...
        params={
            "asset_cfg": SceneEntityCfg("ball"),
            "target_xy": (RIGHT_NEAR_NET_TARGET_X, 0.0),
            "table_center": TABLE_CENTER, "table_size": TABLE_SIZE,
    ...
    right_table_bounce = RewTerm(
        func=mdp.right_table_bounce_reward, weight=250.0,
        params={
            "asset_cfg": SceneEntityCfg("ball"),
            "robot_cfg": SceneEntityCfg("robot", body_names=["paddle_link"]),
            "table_center": TABLE_CENTER, "table_size": TABLE_SIZE,
            ...
            "illegal_reward_scale": 0.0, "require_clean_over_net": False,
            "home_side": "left",
        },
    )
    ...
    table_collision = RewTerm(
        func=mdp.robot_table_penalty, weight=-15.0,
